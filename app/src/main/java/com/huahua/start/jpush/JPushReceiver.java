package com.huahua.start.jpush;
...
import com.huahua.start.activity.HomeActivity;
...
// 获取推送过来的数据
        ...
        if (JPushInterface.ACTION_NOTIFICATION_RECEIVED.equals(intent.getAction())) {
            // 这个推送只是向用户提示信息的，不需要跳转
            int notifactionId = bundle.getInt(JPushInterface.EXTRA_NOTIFICATION_ID);
            Log.d(TAG, "[JPushReceiver] 接收到推送下来的通知的ID: " + notifactionId);

        } else if (JPushInterface.ACTION_NOTIFICATION_OPENED.equals(intent.getAction())) {

            // 这种类型的推送是需要进行跳转处理（ACTION_NOTIFICATION_OPENED）
            ...
            PushMessage pushMessage = (PushMessage) gson.fromJson(bundle.getString(JPushInterface.EXTRA_EXTRA), PushMessage.class);
            /**
             * 判断应用是否处于启动状态
             * 如果应用已经启动(无论前台还是后台)
             */

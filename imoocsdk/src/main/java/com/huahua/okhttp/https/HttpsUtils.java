package com.huahua.okhttp.https;
...
public class HttpsUtils {
    ...
    // 对指定证书的信任
    // 这些步骤都是死的 也不用记 只需要用的时候知道这些步骤干嘛用的即可
    ...
    public static SSLSocketFactory initSSLSocketFactory() {
        // 2.创建加密上下文 sslContext
        SSLContext sslContext = null;
        try {
            // 跟服务器保持一致的算法类型 有SSL TSL，不一致 数据就会出现问题。
            sslContext = SSLContext.getInstance("SSL");
            // 1.生成一个信任管理器类X509TrustManager（一个空实现即可）。
